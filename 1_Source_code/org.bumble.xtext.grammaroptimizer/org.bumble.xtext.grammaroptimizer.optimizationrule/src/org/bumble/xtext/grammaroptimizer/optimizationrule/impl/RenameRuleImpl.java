...
public class RenameRuleImpl extends OptimizationRuleImpl implements RenameRule {
	...
	public void apply() {
		for (GrammarRule rule : grammar.getRules()) {
			if (rule.getName() != null && rule.getName().equals(currentName)) {
				for (LineEntry line : rule.getLines()) {
					if (RegexHelper.doesStringExist(line.getLineContent(), currentName)) {
						if (RegexHelper.doesStringExist(line.getLineContent(), "returns")) {
							line.setLineContent(line.getLineContent().replaceAll(currentName + "\\sreturns", newName + " returns"));
						}
						else {
							line.setLineContent(line.getLineContent().replaceAll(currentName, newName));
						}
